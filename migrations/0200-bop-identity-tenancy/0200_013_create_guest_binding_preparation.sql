...
CREATE UNIQUE INDEX guest_session_binding_scope_key
  ON bop_identity.guest_session (brand_id, store_id, guest_session_id);

CREATE TABLE bop_identity.guest_binding_preparation (
  ...
  FOREIGN KEY (operation_id, prior_revision) REFERENCES bop_identity.guest_binding_preparation (operation_id, revision),
  ...
  CHECK ((
    ...
    AND record ?& ARRAY['operationReference','targetReference','predecessor','candidate','recoverySelectorHash','revision','status','preparedAt','expiresAt','acknowledgedAt','activatedAt']
    AND record - ARRAY['operationReference','targetReference','predecessor','candidate','recoverySelectorHash','revision','status','preparedAt','expiresAt','acknowledgedAt','activatedAt'] = '{}'::jsonb
    AND record->>'operationReference' = operation_id::text
    AND record->>'targetReference' = target_id::text
...
CREATE INDEX guest_binding_preparation_scope_idx
  ON bop_identity.guest_binding_preparation (brand_id, store_id, operation_id, revision DESC);
CREATE UNIQUE INDEX guest_binding_preparation_candidate_idx
  ON bop_identity.guest_binding_preparation (candidate_id) WHERE revision = 1;

CREATE FUNCTION bop_identity.validate_guest_binding_preparation()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
  ...
  IF NEW.revision = 1 THEN
    ...
    END IF;
  ELSE
    SELECT record INTO prior FROM bop_identity.guest_binding_preparation
...
END;
$$;
REVOKE ALL ON FUNCTION bop_identity.validate_guest_binding_preparation() FROM PUBLIC;
CREATE TRIGGER guest_binding_preparation_validate BEFORE INSERT ON bop_identity.guest_binding_preparation
  FOR EACH ROW EXECUTE FUNCTION bop_identity.validate_guest_binding_preparation();
CREATE TRIGGER guest_binding_preparation_no_mutation BEFORE UPDATE OR DELETE ON bop_identity.guest_binding_preparation
  FOR EACH ROW EXECUTE FUNCTION bop_identity.reject_guest_session_operation_mutation();
CREATE TRIGGER guest_binding_preparation_no_truncate BEFORE TRUNCATE ON bop_identity.guest_binding_preparation
  FOR EACH STATEMENT EXECUTE FUNCTION bop_identity.reject_guest_session_operation_mutation();
ALTER TABLE bop_identity.guest_binding_preparation ENABLE ROW LEVEL SECURITY;
ALTER TABLE bop_identity.guest_binding_preparation FORCE ROW LEVEL SECURITY;
CREATE POLICY guest_binding_preparation_scope_policy ON bop_identity.guest_binding_preparation
  USING (brand_id = platform_helpers.current_brand_id() AND store_id = platform_helpers.current_store_id())
  WITH CHECK (brand_id = platform_helpers.current_brand_id() AND store_id = platform_helpers.current_store_id());
REVOKE ALL ON TABLE bop_identity.guest_binding_preparation FROM PUBLIC;
