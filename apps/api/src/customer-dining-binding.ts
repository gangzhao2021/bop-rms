...
export const customerDiningBindingRoutes = Object.freeze({
...
export interface CustomerDiningBindingPort {
  prepare(input: Credentials & { readonly admissionReference: string }): Promise<unknown>;
  activate(
    input: Credentials & {
...
type Action = keyof typeof customerDiningBindingRoutes;
function stagedName(operation: string) {
  return `__Host-bop-guest-dining-candidate-${operation}`;
}
...
function error(response: Response, invalid = false) {
  response.status(invalid ? 400 : 503).json({
    error: {
      code: invalid ? "dining_binding_request_invalid" : "dining_binding_unavailable",
      messageKey: invalid
        ? "customer.dining.binding_request_invalid"
        : "customer.dining.binding_unavailable",
    },
  });
}
...
export const unavailableCustomerDiningBindingHandler: RequestHandler = (_request, response) => {
  controls(response);
  error(response);
};

/** Credential transport only. The supplied Identity service owns current authorization and all effects. */
export class CustomerDiningBindingHandler {
  readonly #origin: string;
  readonly #port: CustomerDiningBindingPort;
  readonly #now: () => string;
  constructor(options: {
    allowedOrigin: string;
    port: CustomerDiningBindingPort;
    now?: () => string;
  }) {
  ...
  #handler(action: Action): RequestHandler {
    return async (request, response) => {
      controls(response);
      let admissionReference: string | null = null;
      ...
      try {
        if (
          request.method !== "POST" ||
          request.originalUrl !== customerDiningBindingRoutes[action] ||
        ...
        const body = readClosedRecord(
          request.body,
          action === "activate"
            ? ["candidateCsrfToken", "recoveryProof"]
            : action === "prepare"
              ? ["admissionReference"]
              : [],
        );
        if (action === "prepare")
          admissionReference = parseGuestDiningAdmissionReference(body.admissionReference);
        if (action === "activate") {
      ...
      try {
        if (action === "prepare") {
          if (admissionReference === null) throw new Error("missing admission");
