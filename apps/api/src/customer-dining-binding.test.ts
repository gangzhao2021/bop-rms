...
import {
  ...
  type CustomerDiningBindingPort,
...
const staged = (operation = op) => `__Host-bop-guest-dining-candidate-${operation}=${candidate}`;
...
function port() {
  return {
    prepare: vi
      .fn<CustomerDiningBindingPort["prepare"]>()
      .mockImplementation(async (input) => prepared(input.operationReference)),
    activate: vi
      .fn<CustomerDiningBindingPort["activate"]>()
      .mockImplementation(async (input) => activated(input.operationReference)),
    complete: vi
      .fn<CustomerDiningBindingPort["complete"]>()
      .mockImplementation(async (input) => activated(input.operationReference)),
  };
}
async function listen(service?: CustomerDiningBindingPort) {
...
async function post(
  server: number,
  action: keyof typeof customerDiningBindingRoutes,
  body: unknown = action === "prepare" ? { admissionReference: admission } : {},
    ...
    const request = httpRequest(
      {
        host: "127.0.0.1",
        port: server,
        path: customerDiningBindingRoutes[action] + query,
        method: "POST",
        headers: {
          origin,
...
describe("same-origin binding credential transport", () => {
  ...
  it("keeps distinct operation cookies for concurrent preparation responses", async () => {
    ...
    const responses = await Promise.all([
      post(server, "prepare"),
      post(server, "prepare", { admissionReference: admission }, { "idempotency-key": other }),
    ]);
  ...
  it("activates from the exact staged credential and clears only that stage", async () => {
    ...
    expect(response.headers.getSetCookie()).toEqual([
      ...
      `__Host-bop-guest-dining-candidate-${op}=; Path=/; Secure; HttpOnly; SameSite=Lax; Max-Age=0`,
    ]);
  ...
  it("does not accept Pickup candidate cookies for Dining activation", async () => {
    const service = port();
    const response = await post(
      await listen(service),
      "activate",
      { candidateCsrfToken: csrf, recoveryProof: proof },
      { cookie: `${main}; __Host-bop-guest-candidate-${op}=${candidate}` },
    );
    expect(response.status).toBe(400);
    expect(service.activate).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    [],
    {},
    { admissionReference: "invalid" },
    { admissionReference: admission, storeReference: op },
    { cartReference: op },
    { sessionCredential: candidate },
  ])("rejects missing, extra or malformed preparation bodies", async (body) => {
    const service = port();
  ...
  it.each([
    ...
    expect(() => new CustomerDiningBindingHandler({ allowedOrigin, port: port(), now })).toThrow();
  });
});
