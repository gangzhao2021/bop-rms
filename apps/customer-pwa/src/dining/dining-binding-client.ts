export type DiningBindingClientErrorCode =
  "request-invalid" | "offline" | "unavailable" | "outcome-unknown";

export class DiningBindingClientError extends Error {
  constructor(readonly code: DiningBindingClientErrorCode) {
    super("dining binding is unavailable");
    this.name = "DiningBindingClientError";
  }
}
export interface DiningBindingPrepared {
...
export interface DiningBindingActivated {
  readonly status: "Activated";
  readonly operationReference: string;
  readonly csrfToken: string;
}
export interface DiningBindingClient {
  prepare(input: unknown): Promise<DiningBindingPrepared>;
  activate(input: unknown): Promise<DiningBindingActivated>;
  complete(input: unknown): Promise<DiningBindingActivated>;
}
...
const routes = Object.freeze({
  prepare: "/bff/customer/dining-binding/prepare",
  activate: "/bff/customer/dining-binding/activate",
  complete: "/bff/customer/dining-binding/complete",
});
function invalid(): never {
  throw new DiningBindingClientError("request-invalid");
}
...
export function createBrowserDiningBindingClient(
  options: {
  ...
  async function send<T>(
    ...
    try {
      if (!online()) throw new DiningBindingClientError("offline");
    } catch {
      throw new DiningBindingClientError("offline");
    }
    ...
    const timeout = new Promise<never>((_resolve, reject) => {
      timer = setTimeout(() => {
        reject(new DiningBindingClientError("outcome-unknown"));
        controller.abort();
      }, timeoutMs);
    });
    const operation = async () => {
      ...
      if (
        response.status === 400 &&
        error.code === "dining_binding_request_invalid" &&
        error.messageKey === "customer.dining.binding_request_invalid"
      )
        knownFailure = "request-invalid";
      if (
        response.status === 503 &&
        error.code === "dining_binding_unavailable" &&
        error.messageKey === "customer.dining.binding_unavailable"
      )
    ...
    try {
      ...
      throw new DiningBindingClientError(knownFailure ?? "outcome-unknown");
    } finally {
      clearTimeout(timer);
      controller.abort();
      if (reader !== undefined) {
    ...
    try {
      const raw = exact(
        ...
        action === "activate"
          ...
          : action === "prepare"
            ? ["operationReference", "csrfToken", "admissionReference"]
            : ["operationReference", "csrfToken"],
      );
      const base = {
        operationReference: reference(raw.operationReference),
        csrfToken: credential(raw.csrfToken),
        admissionReference: action === "prepare" ? reference(raw.admissionReference) : null,
      };
  ...
  return Object.freeze({
    async prepare(value: unknown) {
      ...
      return send(
        ...
        parsed.csrfToken,
        { admissionReference: parsed.admissionReference },
        (value) => {
