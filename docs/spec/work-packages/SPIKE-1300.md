...
| API stability         | [API versioning](https://docs.stripe.com/api/versioning) supports an explicit `Stripe-Version`; the observed GA reference identifies `2026-02-25.clover`.                                                                                                                | The first infrastructure adapter that invokes Stripe must pin an exact GA API/SDK pair and fixtures; no preview channel or implicit account default. Revalidate immediately before that implementation. WP-1301 is a Provider-neutral interface and intentionally imports no SDK. |
...
7. An exact stable GA API version and compatible SDK version are pinned in the first infrastructure
   adapter WP that invokes Stripe. WP-1301 imports no SDK because it defines only the
   Provider-neutral port and normalized synthetic contract. Upgrades require fixtures and explicit
   review; preview/beta versions are prohibited from the baseline.
...
- Final-head GitHub CI: `PASS` on `263072580e8d8a64fba20eff026e5179f76c79df`
  (`bootstrap` run `30817709426`, `verify` job `91699469986`, `14m40s`).
- Squash merge: `main@70cb012ed0bd30a87408cf5197dc7f5cf4a24218` via PR `#137`.
- Exact-main GitHub CI: `PASS` (`bootstrap` run `30818902520`, `verify` job `91703502013`,
  `13m44s`).
