...
import type { DiningGuestBindingOptions } from "../../application/dining-guest-binding-query.js";
...
export function createPostgresDiningGuestBindingStore(
  runner: DiningTableTransactionRunner,
  scopeInput: DiningTableStoreScope,
): DiningGuestBindingOptions["repository"] {
    ...
    async readCurrent(value) {
      ...
      try {
        return await runner.run(async (tx) => {
          ...
          const found = rows(
            await tx.query(
              "SELECT s.session_snapshot AS session,p.participant_snapshot AS participant,t.table_snapshot AS table,a.admission_snapshot AS admission FROM rms_dining.dining_session s JOIN rms_dining.dining_participant p ON p.tenant_id=s.tenant_id AND p.brand_id=s.brand_id AND p.store_id=s.store_id AND p.session_id=s.session_id JOIN rms_dining.dining_identity_admission a ON a.tenant_id=p.tenant_id AND a.brand_id=p.brand_id AND a.store_id=p.store_id AND a.session_id=p.session_id AND a.participant_id=p.participant_id JOIN rms_dining.dining_table t ON t.tenant_id=s.tenant_id AND t.brand_id=s.brand_id AND t.store_id=s.store_id AND t.table_id=s.table_id WHERE s.tenant_id=$1 AND s.brand_id=$2 AND s.store_id=$3 AND s.session_id=$4 AND p.participant_id=$5",
              [tenant, brand, store, input.session, input.participant],
            ),
          );
          if (found.length === 0) return null;
          const raw = closed(found[0], ["session", "participant", "table", "admission"]);
          ...
          if (
            ...
            admission.diningSessionReference !== input.session ||
            admission.participantReference !== input.participant ||
            admission.storeReference !== store ||
          ...
          return Object.freeze({ session, participant, table, admission });
        });
      } catch {
        return sessionDependency();
      }
