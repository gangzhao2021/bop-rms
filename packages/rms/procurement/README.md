...
- Module: `@rms/procurement`; RMS Procurement & Supplier Domain; WP-2131–2137; Procurement
  Engineering Owner.
- Owns Brand-scoped Supplier identity/lifecycle, contact/address references and versioned Supplier
  ...
  It owns the derived Supplier Performance projection and independent append-only Manual Assessments.
...
`procurement_purchase_order_v1`, `procurement_discrepancy_v1` and
`procurement_supplier_performance_v1` provide permission-trimmed reads. Commands require Tenant, Brand,
...
Supplier Performance derives exact rates and coverage from public PO, Revision, Acknowledgement and
Goods Receipt facts. Recalculation and low scores never alter source facts or Supplier lifecycle;
review tasks are explicit audited handoffs and Manual Assessments remain separate from system metrics.
...
external adapter is implemented in WP-2131–2137.
