...
export interface GuestBindingPreparation {
...
export interface GuestBindingProof {
  readonly sessionSelectorHash: GuestSelectorHash;
  readonly csrfSelectorHash: GuestSelectorHash;
  readonly recoverySelectorHash: GuestSelectorHash;
}

export type GuestBindingHashEquals = (left: GuestSelectorHash, right: GuestSelectorHash) => boolean;

/** Supplied by an authenticated owner port, never accepted from a browser. */
export interface GuestBindingOwnerEvidence {
  readonly operationReference: string;
  readonly targetReference: string;
...
const immutableContext = (record: GuestSessionRecord) => {
  ...
  return JSON.stringify([
    ...
    session.diningParticipantReference,
  ]);
};

export function parseGuestBindingPreparation(value: unknown): GuestBindingPreparation {
  return safe(() => {
    ...
    const targetReference = parseGuestSessionReference(raw.targetReference);
    ...
    return Object.freeze({
      ...
      status: raw.status as GuestBindingPreparation["status"],
      preparedAt,
      expiresAt,
      acknowledgedAt,
      activatedAt,
...
export function prepareGuestBinding(input: {
  readonly operationReference: string;
  readonly targetReference: string;
  readonly predecessor: GuestSessionRecord;
  readonly candidate: GuestSessionRecord;
...
function currentAuthority(
  preparation: GuestBindingPreparation,
...
export function acknowledgeGuestBinding(
  input: {
    readonly preparation: GuestBindingPreparation;
...
export function activateGuestBinding(
  input: {
    readonly preparation: GuestBindingPreparation;
    readonly current: GuestSessionRecord;
    readonly proof: GuestBindingProof;
  ...
  return safe(() => {
    ...
    if (
      ...
      evidence.targetReference !== preparation.targetReference ||
      evidence.sessionReference !== candidate.session.sessionReference ||
      evidence.brandReference !== candidate.session.brandReference ||
      evidence.storeReference !== candidate.session.storeReference ||
      evidence.bindingVersion !== 1 ||
      preparation.acknowledgedAt === null ||
      parseCanonicalInstant(evidence.preparedAt) < preparation.acknowledgedAt ||
      parseCanonicalInstant(evidence.preparedAt) > at ||
      parseCanonicalInstant(evidence.validUntil) <= at
    )
      return unavailable();
...
/** Response-loss continuation authenticates the active candidate, never the consumed recovery proof. */
export function completeGuestBinding(
  input: {
    readonly preparation: GuestBindingPreparation;
    readonly current: GuestSessionRecord;
    readonly sessionSelectorHash: GuestSelectorHash;
