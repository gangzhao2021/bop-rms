...
describe("WP-2234 binding proof crypto", () => {
  it("generates 256-bit proof and separates its purpose from Session and CSRF", () => {
    const key = new Uint8Array(32).fill(7);
    const provider = createGuestBindingCredentialProvider(key);
    ...
    const expected = createHmac("sha256", key)
      .update("bop-rms:guest-binding:v1:recovery")
  ...
  it.each([0, 31, 33])("rejects invalid key size %s", (size) => {
    expect(() => createGuestBindingCredentialProvider(new Uint8Array(size))).toThrowError(
