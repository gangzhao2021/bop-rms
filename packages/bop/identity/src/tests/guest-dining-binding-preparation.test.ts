...
import {
  ...
  type GuestDiningBindingOwnerEvidence,
...
function fixture() {
  ...
  const candidate = createGuestSessionRecord({
    ...
    session: {
      ...
      diningParticipantReference: id(11),
      createdAt: at(0),
      lastSeenAt: at(0),
  ...
  const evidence = parseGuestDiningAdmissionEvidence({
  ...
  const activate = (value = acknowledge(), ownerEvidence: unknown = evidence) =>
    activateGuestDiningBinding(
      {
        preparation: value,
        current: predecessor,
        proof,
        ownerEvidence: ownerEvidence as GuestDiningBindingOwnerEvidence,
        observedAt: at(3),
      },
      equals,
    );
...
describe("WP-2294 Dining binding preparation", () => {
  ...
  it("recovers repeated response loss using the active candidate without changing the participant binding", () => {
    const f = fixture();
    const plan = f.activate();
    for (const seconds of [4, 301, 500]) {
      const receipt = completeGuestDiningBinding(
        {
  ...
  it("acknowledgement retries preserve their original time and revision", () => {
    ...
    const retry = acknowledgeGuestDiningBinding(
  ...
  it.each([
    ...
    const f = fixture();
    expect(() => parseGuestDiningBindingPreparation({ ...f.preparation, ...changes })).toThrowError(
      "guest session is unavailable",
    );
  });
  ...
  it("rejects cross-scope candidate and never invokes stored accessors", () => {
    ...
    expect(() => parseGuestDiningBindingPreparation(hostile)).toThrowError(
      "guest session is unavailable",
    );
    expect(reads).toBe(0);
  });
...
describe("WP-2294 closed Dining transition commands", () => {
  ...
  const transition = (name: (typeof names)[number]) => {
    ...
    if (name === "prepare") {
      return {
        command: {
          ...
          admissionReference: f.preparation.admissionReference,
        ...
        run: (input: unknown) =>
          prepareGuestDiningBinding(input as Parameters<typeof prepareGuestDiningBinding>[0]),
      };
    }
    if (name === "acknowledge") {
      return {
        ...
        run: (input: unknown, compare = comparator) =>
          acknowledgeGuestDiningBinding(
            input as Parameters<typeof acknowledgeGuestDiningBinding>[0],
            compare,
          ),
      };
    }
    if (name === "activate") {
      return {
        ...
        run: (input: unknown, compare = comparator) =>
          activateGuestDiningBinding(
            input as Parameters<typeof activateGuestDiningBinding>[0],
            compare,
          ),
      };
    }
    return {
      ...
      run: (input: unknown, compare = comparator) =>
        completeGuestDiningBinding(
          input as Parameters<typeof completeGuestDiningBinding>[0],
          compare,
        ),
    };
  };
...
describe("WP-2294 exact Dining purpose and continuity", () => {
  it.each(["PickupCartBinding", "DiningJoin", "", null])(
    "denies foreign preparation purpose %s",
    (purpose) => {
      const f = fixture();
      expect(() => parseGuestDiningBindingPreparation({ ...f.preparation, purpose })).toThrowError(
        "guest session is unavailable",
      );
    },
  );
  it("requires a ContextOnly predecessor and DiningBound candidate", () => {
    const f = fixture();
    for (const changes of [
      { predecessor: f.candidate },
      {
        candidate: {
          ...f.candidate,
          session: {
      ...
      expect(() =>
        parseGuestDiningBindingPreparation({ ...f.preparation, ...changes }),
      ).toThrowError("guest session is unavailable");
  });
  it("keeps public Table context exact without treating it as an internal Dining Session", () => {
    const f = fixture();
    expect(f.candidate.session.publicTableReference).not.toBe(
      f.candidate.session.diningSessionReference,
    );
    expect(() =>
      parseGuestDiningBindingPreparation({
        ...f.preparation,
        candidate: {
          ...f.candidate,
          session: { ...f.candidate.session, publicTableReference: id(99) },
        },
      }),
    ).toThrowError("guest session is unavailable");
  });
  it.each(["diningSessionReference", "diningParticipantReference"])(
    "completion denies changed %s",
    (key) => {
      const f = fixture();
      const plan = f.activate();
      const current = createGuestSessionRecord({
        ...plan.candidate,
        session: { ...plan.candidate.session, [key]: id(99) },
      });
      expect(() =>
        completeGuestDiningBinding(
          {
  ...
  it("does not re-acknowledge or re-activate an activated preparation", () => {
    const f = fixture();
    const plan = f.activate();
    expect(() => f.acknowledge(plan.preparation)).toThrowError("guest session is unavailable");
    expect(() => f.activate(plan.preparation)).toThrowError("guest session is unavailable");
  });
  it("requires expiry within the predecessor's current deadline", () => {
    const f = fixture();
    const predecessor = createGuestSessionRecord({
      ...f.predecessor,
      session: {
        ...
        createdAt: at(-14400),
        lastSeenAt: at(-14300),
        idleExpiresAt: at(100),
        absoluteExpiresAt: at(72000),
      },
    });
    expect(() =>
      parseGuestDiningBindingPreparation({ ...f.preparation, predecessor }),
    ).toThrowError("guest session is unavailable");
    expect(
      parseGuestDiningBindingPreparation({ ...f.preparation, predecessor, expiresAt: at(100) })
        .expiresAt,
    ).toBe(at(100));
  });
});
