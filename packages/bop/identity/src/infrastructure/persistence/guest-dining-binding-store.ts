import {
  ...
  type GuestDiningBindingProof,
  type GuestDiningBindingOwnerEvidence,
  type GuestDiningBindingHashEquals,
...
export interface GuestDiningBindingAuditDescriptor {
...
export interface GuestDiningBindingAuditPort {
  ...
  append(
    ...
    descriptor: GuestDiningBindingAuditDescriptor,
  ): Promise<void>;
}
...
function captureProof(value: unknown): GuestDiningBindingProof {
...
function captureOwner(value: unknown): GuestDiningBindingOwnerEvidence {
  return parseGuestDiningAdmissionEvidence(value);
}
...
export function createPostgresGuestDiningBindingStore(
  ...
  const scoped = (value: unknown) => {
    const record = parseGuestDiningBindingPreparation(value);
    if (
  ...
  async function load(tx: GuestSessionEntryTransaction, operation: string) {
    const result = rows(
      await tx.query(
        `SELECT record FROM bop_identity.guest_dining_binding_preparation
  ...
  async function append(
    ...
    const result = rows(
      await tx.query(
        `INSERT INTO bop_identity.guest_dining_binding_preparation
  ...
  return Object.freeze({
    async prepare(value: GuestDiningBindingPreparation, observedAt: CanonicalInstant) {
    ...
    async acknowledge(input: {
      ...
      return run(async (tx) => {
        ...
        const next = acknowledgeGuestDiningBinding(
    ...
    async activate(input: {
      ...
      return run(async (tx) => {
        ...
        const inserted = rows(
          ...
          dining_state,status,created_at,last_seen_at,idle_expires_at,absolute_expires_at,version,rotated_from_guest_session_id,dining_session_id,dining_participant_id)
          VALUES ($1,decode($2,'hex'),decode($3,'hex'),$4,decode($5,'hex'),$6,$7,$8,$16,'DineIn',$9,$10,$11,
          'DiningBound','Active',$12,$12,$13,$14,1,$15,$17,$18) RETURNING guest_session_id`,
            [
              s.sessionReference,
