...
Owns the WP-0101 Brand and Store aggregate contract, the WP-0103 permission-free Tenant
Context value, and the `bop_tenant` schema.
...
- Tenant Context revalidates an active Workforce Actor, active Brand, optional active Store
  belonging to that Brand, and a server-owned canonical instant. It carries no Role,
  Permission, Grant, token, Provider subject or Session fact.
...
The package publishes only its root contract. Consumers must not query its private tables.
Resolving Tenant Context proves where an Actor has active membership; it does not authorize an
action. Real Brand/Store facts and Provider data are External Evidence and are not supplied here.
