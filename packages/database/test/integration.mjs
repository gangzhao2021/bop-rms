...
const temp = await mkdtemp(path.join(localRoot, "wp0020-integration-"));
const project = `bop-rms-wp0020-verify-${process.pid}`;
const port = Number.parseInt(process.env.BOP_RMS_WP0020_VERIFY_PORT ?? "55434", 10);
const adminDatabase = "bop_rms_wp0020_admin";
const user = "bop_rms_wp0020_runner";
const password = `wp0020-synthetic-${process.pid}`;
...
assert.match(project, /^bop-rms-wp0020-verify-[0-9]+$/u);
...
function identifier(value) {
  assert.match(value, /^bop_rms_wp0020_[a-z0-9_]+$/u);
  return `"${value}"`;
}
...
async function createDatabase(label) {
  const database = `bop_rms_wp0020_${process.pid}_${label}`;
  identifier(database);
  await admin.query(
...
async function targetClient(database) {
  const client = new Client({
    application_name: "bop-rms-wp0020-integration",
    database,
    host: "127.0.0.1",
    password,
    port,
  ...
  if (extraSql)
    await writeFile(
      path.join(
        ...
        "migrations",
        "0000-platform",
        "0000_002_create_synthetic_rollback_probe.sql",
      ),
      extraSql,
    );
...
try {
  ...
  admin = new Client({
    application_name: "bop-rms-wp0020-integration-admin",
  ...
  assert.deepEqual(initial.pending, ["0000_001_create_migration_history"]);
  ...
  assert.deepEqual(applied.applied, ["0000_001_create_migration_history"]);
  ...
  await lifecycleClient.query(
    `INSERT INTO platform_core.migration_history
     ...
     VALUES ('0000_002_alter_orphan_history', 0, 2,
       'migrations/0000-platform/0000_002_alter_orphan_history.sql',
...
`,
  ...
  assert.deepEqual(rollbackState.rows[0], { history_count: 1, probe: null });
...
`,
  ...
  await writeFile(wrongSecret, "wp0020-secret-must-not-leak", { mode: 0o600 });
  ...
  assert(!`${rejected.stdout}${rejected.stderr}`.includes("wp0020-secret-must-not-leak"));
  assert(!`${rejected.stdout}${rejected.stderr}`.includes(password));
  assert(!`${rejected.stdout}${rejected.stderr}`.includes("SELECT"));

  process.stdout.write(
    "WP-0020 migration integration passed: bootstrap, no-op, drift, rollback, locks, CLI, redaction, and cleanup\n",
  );
} finally {
  await cleanup();
}
